...
- 2026-08-19 — E7.2 — **BLOCKED on founder approval (by design, rule 10)**: the live eval spends ~$10–40 equivalent of subscription usage. Awaiting explicit budget approval; run with `node eval/run.mjs --approved` alongside a running gateway, then RECEIPTS.md gets written from results.jsonl with n/variance/task-class caveats and defaults get re-examined.
